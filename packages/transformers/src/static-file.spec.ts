...
describe('StaticFileTransformer', () => {
  ...
  describe('from', () => {
    const createFilePath = (value: string): string => {
      return new StaticFileTransformer({
        dirname: tmpdir(),
      }).to(value)!;
    };
    ...
    it('should throw error when filePath not under dirname', () => {
      ...
      expect(() => {
        ...
        `${tmpdir()}/372ea08cab33e71c02c651dbc83a474d32c676ea is not under the directory.`,
      );
    });
  ...
  describe('to', () => {
    ...
    it('should return filePath with txt extension', () => {
      ...
      const transformer = new StaticFileTransformer({
        dirname,
        filename: (filename: string) => `${filename}.txt`,
      });
      const filePath = transformer.to('test')!;
      expect(filePath.startsWith(dirname)).toBeTruthy();
      expect(filePath).toBe(
        join(dirname, 'a94a8fe5ccb19ba61c4c0873d391e987982fbbd3.txt'),
      );
    });
    ...
    it('should create sub directory', () => {
      ...
      const transformer = new StaticFileTransformer({
        dirname,
        filename: (filename: string) => `sub/${filename}`,
      });
      expect(transformer.to('test')).toBe(
        join(dirname, 'sub/a94a8fe5ccb19ba61c4c0873d391e987982fbbd3'),
      );
    });
    ...
    it('should create new filePath when pass wrong one', () => {
      ...
      expect(transformer.to(`/path/to/text.txt`)).toBe(
        join(dirname, 'de4ef8b5be48a7fe65ace20f202dcf1db27ab6ed'),
      );
    });
    ...
    it('should return filePath when pass one', () => {
      ...
      ).toBe(join(dirname, 'a94a8fe5ccb19ba61c4c0873d391e987982fbbd3'));
    });
  });
  ...
  describe('image test', () => {
    it('should return buffer', () => {
      const transformer = new StaticFileTransformer({
        dirname: tmpdir(),
        filename: (filename: string) => `${filename}.png`,
      });
      const buffer = readFileSync('test-files/image.png');
      const filePath = transformer.to(buffer);
      expect(filePath).toBe(
        join(tmpdir(), '83a0d2a74f0352bed191dbf96fb20b7591c744e3.png'),
      );
